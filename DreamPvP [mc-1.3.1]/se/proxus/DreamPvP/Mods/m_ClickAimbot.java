...
public class m_ClickAimbot extends Base_Mod implements Updates {
	
	public float hitDelay = 0;

	public m_ClickAimbot() {
		super('7', "Click_aimbot", "Makes you hit other players.", KEY_O, "Aura", "[§eP§r] ");
	}
	...
	public void onUpdate() {
		for(Object o : dp.mc.theWorld.playerEntities) {
			...
			boolean checks = e.canEntityBeSeen(ep) && e.getDistanceToEntity(ep) <= 5 && e != ep && !isFriend && dp.mc.thePlayer.isSwinging;

			if(e.getDistanceToEntity(ep) <= 10) {
				hitDelay++;
				if(checks && hitDelay > 6) {
					...
					dp.utils.sendPacket(new Packet18Animation(e, 1));
