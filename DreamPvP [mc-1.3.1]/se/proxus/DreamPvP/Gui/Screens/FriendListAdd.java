...
public class FriendListAdd extends GuiScreen {
	...
	protected void actionPerformed(GuiButton button) {
		if(button.enabled) {
			if(button.id == 2 && !textField1.getText().isEmpty()) {
				DreamPvP.settings.friendArray.add(textField1.getText());
				DreamPvP.mc.displayGuiScreen(new FriendList());
				DreamPvP.files.saveBaseFile(new File(mc.dp.files.baseFolder, "Friends.txt"), mc.dp.settings.friendArray, true);
			} if(button.id == 3) {
				DreamPvP.mc.displayGuiScreen(new FriendList());
				DreamPvP.files.saveBaseFile(new File(mc.dp.files.baseFolder, "Friends.txt"), mc.dp.settings.friendArray, true);
			}
	...
	public void drawScreen(int I1, int I2, float I3) {
		...
		drawCenteredString(fontRenderer, "Friend list add.", width / 2, 10, 0xFFFFFFFF);
