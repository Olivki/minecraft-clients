...
public class ScreenPluginShop extends GuiScreen {
		...
		if (button.enabled) {
			...
			switch (button.id) {
				case 1:
					String[] info = getPluginOnline().getPluginInfo(guiSlot.getSelected());
					try {
						Download download = new Download(info[4].replace("_", "-"), info[0],
								info[0].replace("@", " ") + " " + info[3] + ".jar", getClient()
										.getPluginDirectory(), getPluginDownloads());
	...
	public void drawScreen(final int x, final int y, final float ticks) {
		...
		guiSlot.func_148128_a(x, y, ticks);
