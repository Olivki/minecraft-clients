...
public class Colours {
	...
	public static int getHEX(final int red, final int green, final int blue, final int alpha) {
		return new Color(red, green, blue, alpha).getRGB()
				+ new Color(red, green, blue, alpha).getAlpha();
	}

	public static int
			getHEX(final float red, final float green, final float blue, final float alpha) {
	...
	public static float[] getRGBA(final int hex) {
		return new float[] { (hex >> 16 & 255) / 255.0F, (hex >> 8 & 255) / 255.0F,
				(hex & 255) / 255.0F, (hex >> 24 & 255) / 255.0F, (hex >> 24 & 0xFF) / 255F };
	}
