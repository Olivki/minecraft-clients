...
public class Slider extends Gui implements Component, DraggableComponent {
	...
	public Slider(final Value value, final int x, final int y, final int width, final int height,
		...
		getMod().getClient().getLogger().info("#1 " + getAmount());
		try {
			setAmount((float) (value.getDouble() / getMax()));
		} catch (Exception e) {
			e.printStackTrace();
		}
		getMod().getClient().getLogger().info("#2 " + getAmount());
		init();
	}
	...
	public void draw(final FontRenderer font, final int x, final int y, final float ticks) {
		...
		if (getAmount() > 0) {
			...
			drawRect(getX(), getY(), getX() + sliderX, getY() + getHeight(), 0x90FF5600);
			/*
	...
	public void handleValues() {
		// mod.setAmtInValue(getAmount(), getText());
		// setValue("" + getAmount());
	}
