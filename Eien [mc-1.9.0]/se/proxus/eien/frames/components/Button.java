...
public class Button extends Gui implements Component {
	...
	public void draw(final FontRenderer font, final int x, final int y, final float ticks) {
		...
		drawRect(getX(), getY(), getX() + getWidth(), getY() + getHeight(), 0x50111111);
		if (getState()) {
			drawRect(getX(), getY(), getX() + getWidth(), getY() + getHeight(), 0x90FF5600);
		}
		if (hovering) {
			// getFrame().getClient().getLogger().info("Hovering: " +
			// getText());
