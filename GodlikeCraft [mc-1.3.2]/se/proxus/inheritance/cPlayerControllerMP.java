...
public class cPlayerControllerMP {
	...
	public cPlayerControllerMP(Minecraft par1Minecraft,
	...
	public static void func_78744_a(Minecraft par0Minecraft,
			cPlayerControllerMP par1PlayerControllerMP, int par2, int par3,
			int par4, int par5) {
		...
		if (this.currentGameType.isAdventure()) {
			...
			if (var6 == null) {
				...
				if (!this.currentGameType.isCreative()) {
					...
					if (var9 != null) {
						var9.func_77941_a(var5, var6.blockID, par1, par2, par3,
								this.mc.thePlayer);
	...
	public void clickBlock(int par1, int par2, int par3, int par4) {
		if(mc.dm.hacks.autoTool.getState()) {
			this.mc.dm.utils.blockAutoTool();
		}
		...
		if (!this.currentGameType.isAdventure()) {
			...
			} else if (!this.isHittingBlock || par1 != this.currentBlockX
					|| par2 != this.currentBlockY || par3 != this.currentblockZ) {
				this.netClientHandler.addToSendQueue(new Packet14BlockDig(0,
						par1, par2, par3, par4));
	...
	public void onPlayerDamageBlock(int par1, int par2, int par3, int par4)
    {
        ...
        else
        {
            if (par1 == this.currentBlockX && par2 == this.currentBlockY && par3 == this.currentblockZ)
            {
                ...
                if (this.curBlockDamageMP >= (mc.dm.hacks.miner.getState() ? 1.0F - ((Float)mc.dm.hacks.miner.getOption(Base_Options.MINE_SPEED)).floatValue() : 1.0F))
                {
                    this.isHittingBlock = false;
                    ...
                    this.prevBlockDamageMP = 0.0F;
                    this.stepSoundTickCounter = 0.0F;
                    this.blockHitDelay = ((Integer)mc.dm.hacks.miner.getOption(Base_Options.MINING_DELAY)).intValue();
                }
	...
	public boolean onPlayerRightClick(EntityPlayer par1EntityPlayer,
		...
		if (var13 > 0
				&& Block.blocksList[var13].onBlockActivated(par2World, par4,
						par5, par6, par1EntityPlayer, par7, var9, var10, var11)) {
			var12 = true;
		}
		...
		if (!var12 && par3ItemStack != null
			...
			if (!var14.canPlaceItemBlockOnSide(par2World, par4, par5, par6,
		...
		if (var12) {
			...
			boolean var16 = par3ItemStack.tryPlaceItemIntoWorld(
	...
	public cEntityClientPlayerMP func_78754_a(World par1World) {
		return new cEntityClientPlayerMP(this.mc, par1World, this.mc.session,
	...
	public ItemStack windowClick(int par1, int par2, int par3, boolean par4,
		...
		short var6 = par5EntityPlayer.craftingInventory
				.getNextTransactionID(par5EntityPlayer.inventory);
		ItemStack var7 = par5EntityPlayer.craftingInventory.slotClick(par2,
