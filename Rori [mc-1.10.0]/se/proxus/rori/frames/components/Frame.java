...
import se.proxus.rori.frames.DraggableComponent;
...
public class Frame extends Gui implements Component, DraggableComponent {
	...
	public Component addComponent(final Component component) {
		if (!getLoadedComponents().contains(component)) {
			...
			getClient().getLogger().info("Added the component '" + component.getText()
		...
		for (Frame frame : FrameManager.getInstance().getLoadedFrames()) {
			...
			if (!frame.getState()) {
				if (x >= frame.getX() && x <= frame.getX() + frame.getWidth() && y >= frame.getY()
					...
					/*
					 * getClient().getLogger().info( "#1 " + frame.getText() +
					...
					&& getIndex(frame.getText()) > getIndex(getText())) {
				/*
				 * getClient().getLogger().info("x >= frame.getX(): " + (x >=
				 * frame.getX())); getClient().getLogger().info(
				 * "x <= frame.getX() + frame.getWidth(): " + (x <= frame.getX()
				 * + frame.getWidth()));
				 * getClient().getLogger().info("y >= frame.getY(): " + (y >=
				 * frame.getY())); getClient().getLogger().info(
				 * "y <= frame.getY() + frame.getHeight() + frame.getComponentsSize(): "
				 * + (y <= frame.getY() + frame.getHeight() +
				 * frame.getComponentsSize())); getClient().getLogger().info(
				 * "getIndex(frame.getText()) > getIndex(getText()): " +
				 * (getIndex(frame.getText()) > getIndex(getText())));
				 * getClient().getLogger().info( "#2 " + frame.getText() + " ["
				 * + getIndex(frame.getText()) + ", " + frame.getState() +
				 * "] Obstructed by: " + getText() + " [" + getIndex(getText())
				 * + ", " + getState() + "]");
				 */
				return true;
			}
		}
		// getClient().getLogger().info("#3 Not Obstructed: " + getText());
		return false;
	}
	...
	public void drawBaseBorderedRect(final float x, final float y, final float width,
			final float height, final int hex1, final int hex2) {
	...
	public void drawBorderedRect(final float x, final float y, final float width,
			final float height, final int hex1, final int hex2) {
		GL11.glPushMatrix();
	...
	public void drawBaseBorderedGradientRect(final float x, final float y, final float width,
	...
	public void drawBorderedGradientRect(final float x, final float y, final float width,
			final float height, final int hex1, final int hex2, final int hex3) {
		GL11.glPushMatrix();
		GL11.glScaled(0.5D, 0.5D, 0.5D);
	...
	public void startCut(final int x, final int y, final int width, final int height) {
		GL11.glScissor(x * Tools.getResolution().getScaleFactor(),
				(Tools.getResolution().getScaledHeight() - height)
						* Tools.getResolution().getScaleFactor(),
				(width - x) * Tools.getResolution().getScaleFactor(),
				(height - y) * Tools.getResolution().getScaleFactor());
