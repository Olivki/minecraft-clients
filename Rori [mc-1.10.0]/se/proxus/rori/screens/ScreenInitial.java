package se.proxus.rori.screens;
...
import se.proxus.rori.io.Download;
import se.proxus.rori.io.DownloadManager;
import se.proxus.rori.tools.IOTools;
...
public class ScreenInitial extends GuiScreen {
	...
	public void downloadPlugins() {
		/*
		 * for (String link : getLoadedLinks()) { String[] info =
		 * link.substring(link.lastIndexOf("/") + 1).split("-"); File tempFile =
		 * new File(getClient().getPluginDirectory(), info[0] + " " + info[3]);
		 * if (!tempFile.exists()) { getDownloads().registerDownload(link,
		 * info[0], info[0] + " " + info[3], getClient(),
		 * getClient().getPluginDirectory()); } }
		 */
	}
	...
	public void drawScreen(final int x, final int y, final float ticks) {
		...
		FontRenderer font = Rori.getInstance().getMinecraft().fontRendererObj;
