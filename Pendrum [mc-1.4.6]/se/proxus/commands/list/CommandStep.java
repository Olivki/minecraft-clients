...
public class CommandStep extends BaseCommand {

	public BaseMod mod = null;

	public boolean custom = false;

	public CommandStep(BaseMod mod, boolean custom, String name) {
		super(name, mod.getInfo().getDescription(), "<toggle/bind/height> <<value>>", mod.getInfo().getAuthor());
		this.mod = mod;
		this.custom = custom;
	}
	...
	public void onCommand(String[] var0, String var1) {
		try {
			...
			try {
				if(var0[0].equalsIgnoreCase("height")) {
					if(!(Float.parseFloat(var0[1]) >= 8.0F)) {
						this.mod.setOption("Step_height", Float.parseFloat(var0[1]), true);
						this.addChat('r', "Step height: " + Float.parseFloat(var0[1]));
					} else {
						this.addChat('r', "The value can't be greater then 8.0.");
					}
				}
			} catch(Exception e) {
