...
public class cPlayerControllerMP extends PlayerControllerMP {
	...
	public void clickBlock(int par1, int par2, int par3, int par4) {	
		mc.bytes.utils.onClickBlock(par1, par2, par3, par4);
	...
	public void onPlayerDamageBlock(int par1, int par2, int par3, int par4)
	{
		...
		m_Miner miner = new m_Miner();
		
		if (creativeMode) {
			blockHitDelay = miner.isEnabled() ? 0 : 5;
			netClientHandler.addToSendQueue(new Packet14BlockDig(0, par1, par2, par3, par4));
			PlayerControllerCreative.clickBlockCreative(mc, this, par1, par2, par3, par4);
			return;
		}
		...
		if (par1 == currentBlockX && par2 == currentBlockY && par3 == currentblockZ) {
			...
			curBlockDamageMP += miner.isEnabled() ? block.blockStrength(mc.thePlayer) * 1.2 : block.blockStrength(mc.thePlayer);
			...
			if(Bytes.settings.autoMineBreak == 1.0F) {
				Bytes.settings.autoMineBreak = 0.0F;
			} else {
				Bytes.settings.autoMineBreak += 0.1F;
			}
			
			stepSoundTickCounter++;
			
			if (curBlockDamageMP >= (miner.isEnabled() ? 1.0F - miner.mineSpeed : 1.0F)) {
				isHittingBlock = false;
	...
	public boolean onPlayerRightClick(EntityPlayer par1EntityPlayer, World par2World, ItemStack par3ItemStack, int par4, int par5, int par6, int par7) {
		syncCurrentPlayItem();
		Bytes.utils.onRightClick(par1EntityPlayer, par2World, par3ItemStack, par4, par5, par6, par7);
	...
	public ItemStack windowClick(int par1, int par2, int par3, boolean par4, EntityPlayer par5EntityPlayer) {
		return super.windowClick(par1, par2, par3, par4, par5EntityPlayer);
	}
