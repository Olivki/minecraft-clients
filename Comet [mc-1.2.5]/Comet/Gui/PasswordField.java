...
public class PasswordField extends Gui
{
	...
	public PasswordField(GuiScreen guiscreen, FontRenderer fontrenderer, int i, int j, int k, int l, String s)
	{
		isFocused = false;
		isEnabled = true;
	...
	{
		String regex = "[a-zA-Z0-9 \\p{ASCII}]";
		...
		if (isEnabled)
		{
			...
			drawString(fontRenderer, (new StringBuilder()).append(text.replaceAll(regex, "*")).append(flag ? "_" : "").toString(), xPos + 4, yPos + (height - 8) / 2, 0xe0e0e0);
		}
		else
		{
			drawString(fontRenderer, text.replaceAll(regex, "*"), xPos + 4, yPos + (height - 8) / 2, 0x707070);
		}
	}
