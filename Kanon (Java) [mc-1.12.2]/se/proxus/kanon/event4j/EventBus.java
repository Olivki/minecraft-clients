...
import se.proxus.kanon.event4j.marker.EventMarker;
import se.proxus.kanon.event4j.marker.MarkedEvent;
...
public final class EventBus<E, L> {
    ...
    public void unregister(final L listener) {
        ...
        for (final Method method : listener.getClass().getDeclaredMethods()) {
            if (!eventMarker.isMarked(method)) {
                continue; // Not a handler
            }
            final RegisteredListener<E, L> registeredListener =
                    new RegisteredListener<>(this, method, listener, EventExecutor.empty());
            
            if (!handlers.containsKey(registeredListener.getEventType())) {
                continue;
            }
    ...
    public void register(final L listener) {
        ...
        for (final Method method : listener.getClass().getDeclaredMethods()) {
            ...
            final RegisteredListener<E, L> registeredListener =
                    new RegisteredListener<>(this, method, listener, executorFactory.create(this, method));
    ...
    public boolean isMarked(final Method method) {
        return getEventMarker().isMarked(method);
    }
    ...
    public static class Builder<E, L> {
        ...
        private EventMarker eventMarker = m -> m.isAnnotationPresent(EventSubscribe.class) ? (MarkedEvent) m.getAnnotation(
                EventSubscribe.class).priority()::ordinal : null;
