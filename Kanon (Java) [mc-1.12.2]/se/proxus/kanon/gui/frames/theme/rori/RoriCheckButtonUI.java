...
public class RoriCheckButtonUI extends AbstractComponentUI<CheckButton> {

    private final RoriTheme theme;

    RoriCheckButtonUI(final RoriTheme theme) {
    ...
    protected void handleComponentInteraction(final CheckButton component,
        ...
        if (location.x <= component.getWidth()
                && location.y <= component.getHeight() && button == 0) { component.press(button); }
    }
}
